 ...
 * NDJSON stream.  This module bakes in rpilot's permission policy
 ...
 * yields no query(), which broke rpilot inconsistently across machines
...
//
// Builds the CLI invocation from rpilot's options and the native binary's
...
function runClaude(opts, onMessage) {
  ...
  // Prompt is passed as a positional arg (not stdin) — the Claude binary does
  // not reliably read piped stdin on Windows when invoked with --print.

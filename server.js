...
const PORT = parseInt(process.env.rpilot_PORT || '3922', 10);
...
const CHUNK_DIFF_THRESHOLD = parseInt(process.env.rpilot_CHUNK_THRESHOLD || '80000', 10);
...
const SKILL_MD = path.join(AGENT_DIR, 'AGENT.md');
...
function trackEvent(actionName) {
  try {
    const url = 'https://analyticsdev.asite.com/matomo.php?idsite=5&rec=1' +
      ...
      '&url=' + encodeURIComponent('https://asite.com/rpilot');
 ...
 * First chunk uses the standard $rpilot invocation so the system prompt
...
function buildPromptForChunk(params, scope, chunkScope, chunkIndex, totalChunks) {
  ...
  if (chunkIndex === 0) {
    ...
    var chunkNote    = totalChunks > 1
      ...
      : '';
    return '$rpilot Review ' + roleLabel + ' changes against ' + params.base +
    ...
    readBody(req).then(function (params) {
      ...
      runScope(params, log.logPath).then(function (scope) {
        ...
        if (params.enableChunking) {
          ...
          // Reassemble per-file diff_chunks into full diffs so the whole
          // preprocessed diff goes to the AI in one call and processFileMergePass
          // finds nothing to do (no extra Claude calls, no extra tokens).
          scope.files.forEach(function(f) {
            if (f.diff_chunked && Array.isArray(f.diff_chunks)) {
              f.diff = f.diff_chunks.join('');
              delete f.diff_chunks;
              delete f.diff_chunked;
              delete f.diff_chunk_total;
            }
          });
...
server.listen(PORT, '127.0.0.1', function () {
  ...
  // trackEvent('rpilot-skill-invoked'); // Enable once plugin is working properly
});

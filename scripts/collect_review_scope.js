...
function getUnifiedDiff(repo, diffArgs, file, context) {
  const args = [...diffArgs, `--unified=${context}`, '--ignore-all-space', '--ignore-blank-lines', '--', file];
...
function parseArgs(argv) {
  const args = {
    ...
    max_diff_bytes: parseInt(process.env.rpilot_FILE_CHUNK_BYTES, 10) || 6000, max_files: 0,
    log_file: '',
  };
...
function buildMissingInputsOutput(args) {
  return {
    tool: 'rpilot.collect_review_scope',
    ...
    examples: [
      '$rpilot Review all changes done by the UI developer against the QA base branch',

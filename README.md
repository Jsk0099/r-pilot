...
Copy the whole `rpilot/` folder into your Claude skills directory — no build step, no dependencies beyond Node.js.

- **Project-level** (only this repo): `.claude/skills/rpilot/`
- **User-level** (all your projects): `~/.claude/skills/rpilot/`

```bash
# from the unzipped bundle:
mkdir -p .claude/skills
cp -r rpilot .claude/skills/
```

The folder name (`rpilot`) and the `name:` field in `SKILL.md` must match. Restart / reload Claude so it picks up the new skill, then invoke it with `$rpilot ...`. The exact skills path can vary by client — confirm at https://docs.claude.com/en/docs/claude-code/overview if your setup differs.

> Codex compatibility: the same folder also works under `.agents/skills/rpilot`. `openai.yaml` is Codex-only interface metadata and is ignored by Claude.
...
- **Agent UI** (`ui/index.html`) to enter role/extensions, base branch, and extra prompt, with a live step tracker; it generates the exact `$rpilot` prompt and scope command.
